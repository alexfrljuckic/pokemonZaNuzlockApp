...
export function MilestoneProgressBar({ cleared, total }: { cleared: number; total: number }) {
  if (total <= 0) {
    return (
      <div className="chart-block">
        <p className="muted chart-caption">No milestones defined for this dataset yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="chart-block">
      <svg viewBox="0 0 300 24" className="chart-svg chart-svg-wide" role="img" aria-label="Milestone progress bar">
      ...
      <p className="muted chart-caption">
        {cleared}/{total} milestones cleared ({percent}%)
      </p>
    </div>
  );
}
